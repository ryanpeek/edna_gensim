...
N <- 20 # effective pop size
(pop_size_r <- 4*N*ll_mu)

# 1 Populations, 5 indiv
model <- coal_model(sample_size = c(5), loci_number = 5, 
                    loci_length = 100, ploidy = 2) +
  feat_mutation(rate = pop_size_r, model = "IFS") + # rate= 4*N0*mu
  feat_mutation(par_prior("theta", runif(1, .5, 1))) + # add a prior
...
# simulate
sumstats <- simulate(model, seed = 15, nsim=10)

# setup of list of names
(vecNames <- sprintf("s%03d", 1:10))

# now add to list of sims
sumstats <- purrr::set_names(sumstats, nm = vecNames)

# double check
map(sumstats, ~names(.x))

# get just pi
pi_df <- map(sumstats, ~.x["pi"]) %>% flatten() %>% bind_cols() %>% 
  rename_all(., ~vecNames) %>% 
...
map(sumstats, ~pluck(.x[["seg_sites"]][[3]])) %>% simplify()

# test plots
barplot(sumstats[["s001"]][["sfs"]])
plot(sumstats[[1]]$sfs)

# plot SFS
(sfs_df <- map(sumstats, "sfs")  %>% bind_rows())
...
Other notes: 
 ...
 - **ABC** https://cran.r-project.org/web/packages/coala/vignettes/coala-abc.html
